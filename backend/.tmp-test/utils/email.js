...
async function sendEmail({ to, subject, html }) {
    if (!env_1.env.brevoApiKey) {
        logger_1.logger.warn(`[email:console-fallback] BREVO_API_KEY not set. Would have sent to ${to}: "${subject}"`);
    ...
    try {
        const response = await fetch("https://api.brevo.com/v3/smtp/email", {
            ...
            body: JSON.stringify({
                sender: {
                    name: env_1.env.emailFromName,
                    email: env_1.env.emailFrom,
                },
                to: [
                    {
                        email: to,
        ...
        if (!response.ok) {
            ...
            logger_1.logger.error(`Failed to send email to ${to}: ${response.status} ${body}`);
        }
    }
    catch (err) {
        ...
        logger_1.logger.error(`Error sending email to ${to}: ${err.message}`);
    }
}
function wrapper(title, bodyHtml, ctaLabel, ctaUrl) {
    return `
...
async function sendVerificationEmail(to, name, link) {
    await sendEmail({
        ...
        html: wrapper(`Hi ${name}, please verify your email`, `Thanks for creating a WorkPilot workspace. Click the button below to verify <strong>${to}</strong> and activate your account.`, 'Verify email', link),
    });
}
async function sendInviteEmail(to, name, companyName, link) {
    await sendEmail({
        to,
        subject: `You've been added to ${companyName} on WorkPilot`,
        html: wrapper(`Hi ${name}, you've been invited`, `You've been added to <strong>${companyName}</strong>'s workspace on WorkPilot. Click below to verify your email and set your own password.`, 'Set my password', link),
    });
}
async function sendPasswordResetEmail(to, name, link) {
    await sendEmail({
        ...
        html: wrapper(`Hi ${name}, reset your password`, `We received a request to reset your WorkPilot password. This link expires in 1 hour. If you didn't request this, you can safely ignore this email.`, 'Reset password', link),
    });
}
